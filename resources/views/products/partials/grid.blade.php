        ...
        <div class="group bg-white rounded-2xl border border-stone-200 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-200">
            ...
            <a href="{{ route('products.show', $product) }}">
                <div class="relative aspect-square bg-stone-100 flex items-center justify-center overflow-hidden">
                    ...
                    @if ($product->stock == 0)
                        <span class="absolute top-3 left-3 bg-stone-800 text-white text-[11px] font-medium px-2 py-1 rounded-full">Out of stock</span>
                    @elseif ($product->stock <= 5)
                        <span class="absolute top-3 left-3 bg-red-500 text-white text-[11px] font-medium px-2 py-1 rounded-full">Low stock</span>
                    @endif
                </div>

                <div class="p-4 pb-2">
                    <p class="text-[11px] uppercase tracking-wide text-orange-600 font-semibold mb-1">{{ $product->category->name }}</p>
                    <h3 class="font-medium text-stone-800 leading-snug line-clamp-2 mb-2">{{ $product->name }}</h3>
                    <p class="text-lg font-bold text-stone-900">${{ number_format($product->price, 2) }}</p>
                </div>
            </a>
